...
function sampleAcquired(s){   
            ...
            // Get samples from the object - get 0th element of samples as base 64 encoded PNG image         
                ...
                localStorage.setItem("imageSrc", "data:image/png;base64," + window.Fingerprint.b64UrlTo64(samples[0]));
                ...
                localStorage.setItem("raw", window.Fingerprint.b64UrlTo64(decodedData.Data));
                ...
                var sampleData = window.Fingerprint.b64UrlTo64(samples[0].Data);
                var decodedData = JSON.parse(window.Fingerprint.b64UrlToUtf8(sampleData));
                localStorage.setItem("wsq","data:application/octet-stream;base64," + window.Fingerprint.b64UrlTo64(decodedData.Data));
...
function selectChangeEvent(){
    ...
    if(myVal == ""){
        document.getElementById('capabilities').disabled = true
        // $('#capabilities').prop('disabled', true);
    }else{
        document.getElementById('capabilities').disabled = false
...
// document.getElementsByTagName("body").addEventListener('click', function() { disableEnableStartStop() })
...
var dataURItoBlob = function(dataURI, dataURIType) {
...
//                 currentFormat = window.Fingerprint.SampleFormat.Raw;
//             }
//             if(elements[i].name == "Intermediate"){
//                 currentFormat = window.Fingerprint.SampleFormat.Intermediate;
//             }
//             if(elements[i].name == "Compressed"){
//                 currentFormat = window.Fingerprint.SampleFormat.Compressed;
//             }
//             if(elements[i].name == "PngImage"){
...
function disableEnableExport(val){
    if(val){
        document.getElementById('saveImagePng').disabled = true
        // $('#saveImagePng').prop('disabled', true);
    }else{
        document.getElementById('saveImagePng').disabled = false
